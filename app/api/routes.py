...
# Get all patients with unresolved conflicts
...
#  Get patients with unresolved conflicts for a specific clinic
...
#  Get conflicts for a specific patient
...
def resolve_conflict(conflict_id: str, data: dict):

    try:
        result = db.conflicts.update_one(
            {"_id": ObjectId(conflict_id)},
            {
                "$set": {
                    "status": "resolved",
                    "resolution": {
                        "resolved_by": data.get("resolved_by", "clinician"),
